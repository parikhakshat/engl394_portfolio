...
let rec union_finals (finals: 'q list) (new_final: 'q) : ('q, 's) transition list =
  match finals with
  [] -> []
  | h::t -> [(h, None, new_final)] @ union_finals t new_final
...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  let nfa = { qs= [start; final]
  ; sigma= [c]
  ; delta= [(start, (Some c), final)]
  ; q0= start
  ; fs= [final]
} in nfa
  | Union (r1, r2) -> let nfa1 = regexp_to_nfa r1 in 
  let nfa2 = regexp_to_nfa r2 in
  let start = fresh () in
  let final = fresh () in
  let nfa3 = {
    qs = [start] @ [final] @ insert_all (union nfa1.qs nfa2.qs) []
    ; sigma = insert_all (union nfa1.sigma nfa2.sigma) []
    ; q0 = start
    ; fs = [final]
    ; delta= [(start, None, nfa1.q0); (start, None, nfa2.q0)] @ (union_finals nfa1.fs final) @ (union_finals nfa2.fs final) @ insert_all (union nfa1.delta nfa2.delta) []
  } in nfa3
  | Concat (r1, r2) -> let nfa1 = regexp_to_nfa r1 in
  let nfa2 = regexp_to_nfa r2 in
  let nfa3 = {
    qs = insert_all (union nfa1.qs nfa2.qs) []
    ; sigma = insert_all (union nfa1.sigma nfa2.sigma) []
    ; q0 = nfa1.q0
    ; fs = nfa2.fs
    ; delta= (union_finals nfa1.fs nfa2.q0) @ insert_all (union nfa1.delta nfa2.delta) []
  } in nfa3
  | Star r -> let nfa = regexp_to_nfa r in
  let start = fresh () in let final = fresh () in
  let nfa2 = {
    qs = [start] @ [final] @ nfa.qs
    ; sigma = nfa.sigma
    ; q0 = start
    ; fs = [final]
    ; delta= [(start, None, nfa.q0); (start, None, final); (final, None, start)] @ (union_finals nfa.fs start) @ nfa.delta
  } in nfa2;;
